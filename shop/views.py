    ...
    def get_queryset(self):
        ...
        if sort_by:
            ...
            if sort_by == "price":
                ...
                queryset = queryset.order_by(
                    F("created_at").asc(nulls_last=True)
                    if sort_order == "asc"
                    else F("created_at").desc(nulls_last=True)
                )
        if filter_by:
...
class ShopCreate(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Only shop owners can create a shop."}, status=400
            )
...
# shopProps Views: ShopPropsDetail, getShopPropsFromShopId


class ShopPropsList(generics.ListAPIView):
    queryset = ShopProps.objects.all()
    serializer_class = ShopPropsSerializer


class ShopPropsCreate(generics.CreateAPIView):
    queryset = ShopProps.objects.all()
    serializer_class = ShopPropsSerializer


class ShopPropsDetail(generics.RetrieveUpdateDestroyAPIView):
    queryset = ShopProps.objects.all()
    serializer_class = ShopPropsSerializer


class getShopPropsFromShopId(APIView):
    serializer_class = ShopPropsSerializer

    def get(self, request, shopId):
        try:
            print(shopId)
            queryset = ShopProps.objects.filter(shopId=shopId)
            if not queryset:
                return Response({"error": "Shop not found."}, status=400)
            serializer = ShopPropsSerializer(queryset, many=True)
            return Response(serializer.data)
        except Exception as e:
            return Response({"error": str(e)}, status=400)
...
class AddToCart(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            cart.save()
            return Response(
                {"success": "Product added to cart successfully."}, status=200
            )
        except Exception as e:
            return Response({"error": str(e)}, status=400)
